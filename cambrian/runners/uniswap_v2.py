...
def liquidity_usd_from_reserves(reserves: tuple[int, int] | None, *,
    ...
    """Total pool depth in USD, as the quote-side reserve doubled.

    Unlike the v3/v4 estimates this is exact for the quote leg — a V2 pair's
    reserve IS its balance, with no concentration to correct for.
    """
    if not reserves:
        return None
